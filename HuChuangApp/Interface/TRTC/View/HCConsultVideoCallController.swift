...
//                seconds = self.callingTime % 60
                ...
                PrintLog("倒计时：\(self.cutdownCallingTime)")
                PrintLog("显示：\(mins) --- \(seconds)")
    ...
    deinit {
        ...
        HCConsultVideoCallController.renderViews = []
        ...
        NotificationCenter.default.rx.notification(NotificationName.ChatCall.userEnter)
            .subscribe(onNext: { [weak self] no in
                ...
                if let data = no.object as? (CallingUserModel, HCReceivePhoneModel) {
                    ...
                    if !data.1.isCallEnd {
                        self?.isInCalling = true
                        self?.cutdownCallingTime = data.1.seconds
//                        self?.cutdownCallingTime = 8
                        self?.startGCDTimer()
                    }else {
                        TRTCCalling.shareInstance().hangup()
                        self?.resetRemind(text: "通话时长已用完，通话已结束")
...
//        self.codeTimer.cancel()
        ...
        let watchTime = self.isInCalling == true ? "1" : "0"
...
extension HCConsultVideoCallController: UICollectionViewDelegate, UICollectionViewDataSource,
                                   UICollectionViewDelegateFlowLayout {
    
    func resetWithUserList(users: [CallingUserModel], isInit: Bool = false) {
        resetUserList()
        let usersFilter = users.filter {
            let curUser = V2TIMManager.sharedInstance()?.getLoginUser() ?? ""
            return $0.userId != curUser
        }
    ...
    func resetUserList() {
        if let sponsor = curSponsor {
            ...
            if let name = HCHelper.share.userInfoModel?.name,
                let avatar = HCHelper.share.userInfoModel?.headPath,
                let userId = HCHelper.share.userInfoModel?.uid {
                curUser.name = name
    ...
    func enterUser(user: CallingUserModel) {
        if user.userId != V2TIMManager.sharedInstance()?.getLoginUser() ?? "" {
            ...
            HCConsultVideoCallController.renderViews.append(renderView)
    ...
    func leaveUser(user: CallingUserModel) {
        ...
        HCConsultVideoCallController.renderViews = HCConsultVideoCallController.renderViews.filter {
    ...
    func updateUserVolume(user: CallingUserModel) {
        if let firstRender = HCConsultVideoCallController.getRenderView(userId: user.userId) {
...
extension HCConsultVideoCallController {
    func initUI() {
        HCConsultVideoCallController.renderViews = []
