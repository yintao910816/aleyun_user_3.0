    ...
    case CallAction_Call:
        {
            ...
            if (isGroup) {
                ...
                @weakify(self)
                callID = [[V2TIMManager sharedInstance] invite:realModel.invitedList.firstObject data:data onlineUserOnly:false offlinePushInfo:nil timeout:SIGNALING_EXTRA_KEY_TIME_OUT succ:^{
                    @strongify(self)
                    // 发起 Apns 推送
    ...
    case CallAction_End:
        {
            if (isGroup) {
                ...
                if (self.startCallTS > 0) {
                    ...
                    NSString *data = [TRTCCallingUtils dictionary2JsonStr:param];
                    callID = [[V2TIMManager sharedInstance] invite:receiver data:data onlineUserOnly:false offlinePushInfo:nil timeout:0 succ:nil fail:^(int code, NSString *desc) {
                        if ([self canDelegateRespondMethod:@selector(onError:msg:)]) {
                            [self.delegate onError:code msg:desc];
                        };
                    }];
